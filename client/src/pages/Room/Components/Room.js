...
class GameUI extends React.Component {
                        ...
                        if (game.card.isCashCard(card)) {
                            ...
                            if (game.card.isDrawCard(card)) {
                                ...
                                if (isSuccess) {
                                    ...
                                    let speechEnd =
                                        " collections that I can charge rent.";
                                ...
                                let isSuccess =
                                    game.initAskForValueCollection(card);
                                if (isSuccess) {
                                    return true;
                                }
                            } else if (
                                game.card.hasTag(card, "stealProperty")
                            ) {
                                ...
                                let isSuccess =
                                    game.initAskForPropertySwap(card);
                                if (isSuccess) {
                                    return true;
                                }
                            } else if (
                                game.card.hasTag(card, "stealCollection")
                            ) {
                ...
                if (isDef(cardId)) {
                    ...
                    if (dropZone.isEmptySet) {
                        if (item.from === "hand") {
                            sounds.playcard.play();
                            requestResult =
                                await game.addPropertyToNewCollectionFromHand(
                                    cardId
                                );
                            ...
                            if (isDef(fromCollectionId)) {
                                if (card.type === "property") {
                                    sounds.playcard.play();
                                    requestResult =
                                        await game.transferPropertyToNewCollection(
                                            cardId,
                                            fromCollectionId
                                        );
                                } else if (
                                    ...
                                    sounds.build.play();
                                    requestResult =
                                        await game.transferSetAugmentToNewCollection(
                                            cardId,
                                            fromCollectionId
                                        );
                                }
                                ...
                                console.error(
                                    "handleOnCardDrop missing fromCollectionId"
                                );
                            }
                        } else {
                            console.error(
                        ...
                        if (item.from === "hand") {
                            if (game.card.isPropertyCard(card)) {
                                sounds.playcard.play();
                                requestResult =
                                    await game.addPropertyToExistingCollectionFromHand(
                                        cardId,
                                        toCollectionId
                                    );
                                ...
                                sounds.build.play();
                                requestResult =
                                    await game.addAugmentToExistingCollectionFromHand(
                                        cardId,
                                        toCollectionId
                                    );
                            } else if (
                                ...
                                console.error(
                                    "Card is not a property or set augment"
                                );
                            }
                        } else if (item.from === "collection") {
                            let fromCollectionId = item.collectionId;
                            if (isDef(fromCollectionId)) {
                                ...
                                if (card.type === "property") {
                                    sounds.playcard.play();
                                    requestResult =
                                        await game.transferPropertyToExistingCollection(
                                            cardId,
                                            fromCollectionId,
                                            toCollectionId
                                        );
                                ...
                                // Transfer set augment to existing collection
                                    ...
                                    sounds.build.play();
                                    requestResult =
                                        await game.transferSetAugmentToExistingCollection(
                                            cardId,
                                            fromCollectionId,
                                            toCollectionId
                                        );
                ...
                <FillContainer>
                    <FillContent>
                        <FlexColumn style={{ width: "100%", height: "100%" }}>
                            {game.player.opponents
                                .getAll()
                                .map(this.renderPlayerPanel)}
                        </FlexColumn>
                    </FillContent>
                    {game.me() ? (
                        <FillFooter>
                            {this.renderPlayerPanel(game.me())}
                        </FillFooter>
                    ) : (
                        ""
                    )}
    ...
    async handleOnChargeRentClick() {
        if (game.canChargeRent()) {
            let selectedCollectionIds =
                game.selection.collections.selected.get();
    ...
    makeCardCheck({
        ...
        if (isCollection) {
            ...
            if (isDef(collection)) {
                isCollectionSelectable =
                    game.selection.collections.selectable.has(collectionId);
            }
        }
        ...
        function isSelectionEnabled(cardId) {
            ...
            if (
                game.turn.isMyTurn() &&
                game.turn.getPhaseKey() === "done" &&
                isCollection &&
                game.card.isWildPropertyCard(cardId)
            ) {
                return false;
            }
        ...
        function canSelectCard(cardId) {
            ...
            if (isMyHand) {
                ...
                if (actionCardId === cardId) {
                    ...
                    let actionCardIsRent = game.card.hasTag(
                        actionCardId,
                        "rent"
                    );
    ...
    updateRender(customFn, mode) {
        ...
        function beforeOnClick() {
            ...
            let playerTurnOverlay =
                windowManager.getWindowByKey("playerTurnOverlay");
        ...
        function setStealCollectionButton() {
            let buttonEnabled =
                game.selection.collections.selected.isLimitSelected();
            game.updateRenderData(["actionButton", "disabled"], !buttonEnabled);
            game.updateRenderData(
                ["actionButton", "title"],
                "Confirm collection"
        ...
        function setAskForValueButton() {
            let isButtonDisabled =
                !game.selection.people.selected.isLimitSelected();
            game.updateRenderData(
                ["actionButton", "disabled"],
                isButtonDisabled
            );
        ...
        if (isGameOver) {
            ...
            if (isRequestScreenOpen) {
                ...
                let getRequestDeclineCard = (requestId) => {
                    if (declinableCardIds.length > 0)
                        return declinableCardIds[0];
                    return null;
                };
                            ...
                            case "property":
                                if (ids.length > 0) {
                                    ...
                                    ids.forEach((id) => {
                                        ...
                                        let collectionId =
                                            game.getIncompleteCollectionMatchingSet(
                                                game.myId(),
                                                card.set
                                            );
                                        game.collectCardToCollection(
                                            requestId,
                                            id,
                                            collectionId
                                        );
                ...
                if (displayMode === SCREENS.COLLECT) {
                    ...
                    let handleOnCardDrop = ({ dragProps, dropProps }) => {
                        if (
                            isDefNested(dragProps, "is") &&
                            dragProps.is === "collection"
                        ) {
                    ...
                    let handleOnCollectionClick = async ({
                        collectionId,
                        requestId,
                    }) => {
                        return game.collectCollection({ requestId });
                    ...
                    let requestIds = game.requests
                        ...
                        .filter((requestId) => {
                            ...
                            return (
                                String(preformedOnActionNum) ===
                                String(currentActionAcount)
                            );
                        });
                    ...
                    let makeGetterPersonTransferIds =
                        (field, funcName) => (requestId) => {
                            let results = [];
                            let request = game.request.get(requestId);
                            if (isDef(request)) {
                                let direction;
                                let amITarget =
                                    String(thisPersonId) ===
                                    String(request.targetKey);
                                let amIAuthor =
                                    String(thisPersonId) ===
                                    String(request.authorKey);
                                if (amIAuthor) {
                                    direction = "toAuthor";
                                    if (
                                        isDefNested(game, [
                                        ...
                                        let ids =
                                            game.request.transfer[direction][
                                                field
                                            ][funcName](requestId);
                                        ids.forEach((id) => {
                                            results.push(id);
                ...
                if (displayMode === SCREENS.PAY) {
                    ...
                    let actionLabel = getNestedValue(
                        request,
                        "actionLabel",
                        ""
                    );
                    ...
                    let augmentCardIds = getNestedValue(
                        request,
                        "augmentCardIds",
                        []
                    );
                    ...
                    return (
                        ...
                        >
                            <RenderInteractableCard
                                scaledPercent={
                                    uiConfig.hand.default.scalePercent
                                }
                                card={card}
                                clickProps={{
                                    from: "hand",
                                ...
                                onActiveSetChange={async ({
                                    ...
                                    let result =
                                        await game.flipWildPropertyCard(
                                            cardId,
                                            propertySetKey
                                        );
    ...
    renderPlayerPanel(person) {
        ...
        if (isDef(person)) {
            ...
            let playerSelection = "";
            if (
                game.selection.people.isEnabled() &&
                person.id !== game.myId()
            ) {
                ...
                playerSelection = (
                    <FullFlexColumnCenter>
                        <PulseCheckBox
                            ...
                            value={isChecked}
                            onClick={() => {
                                game.selection.people.selected.toggle(
                                    person.id
                                );
                            }}
                        />
            ...
            if (game.getDisplayData(["mode"]) === "askForPropertySwap") {
                ...
                if (person.id !== myid) {
                    ...
                    if (isDef(playersCardsKeyed[myid]))
                        canSelectCardFromUser = false;
                }
            }
                        ...
                        <PlayerPanelContent>
                            ...
                            <CollectionContainer>
                                ...
                                {collectionsForPerson
                                    .map((collectionId) => {
                                        let collection =
                                            game.collection.get(collectionId);
                                        let collectionCards =
                                            game.collection.getCards(
                                                collectionId
                                            );

                                        let collectionSelection = {
                                            enabled:
                                                game.selection.collections.isEnabled(),
                                            selectType:
                                                game.selection.collections.getType(),
                                            isSelectable:
                                                game.selection.collections.selectable.has(
                                                    collectionId
                                                ),
                                            isSelected:
                                                game.selection.collections.selected.has(
                                                    collectionId
                                                ),
                                            isFull: game.collection.isComplete(
                                                collectionId
                                            ),
                                        };
                                            ...
                                            collectionContent = (
                                                ...
                                                >
                                                    <PropertySetContainer
                                                        ...
                                                        transparent={true}
                                                        selectionEnabled={
                                                            collectionSelection.enabled
                                                        }
                                                        selectType={
                                                            collectionSelection.selectType
                                                        }
                                                        isSelectable={
                                                            collectionSelection.isSelectable
                                                        }
                                                        isSelected={
                                                            collectionSelection.isSelected
                                                        }
                                                        onSelected={
                                                            this
                                                                .handleCollectionSelect
                                                            ...
                                                            (card) => {
                                                                return (
                                                                    <ShakeAnimationWrapper
                                                                        ...
                                                                        triggerValue={this.getShakeCardValue(
                                                                            card.id
                                                                        )}
                                                                    >
                                                                        <RenderInteractableCard
    ...
    renderMyArea() {
        ...
        if (
            game.isMyTurn() &&
            ["action", "request"].includes(game.phase.get())
        ) {
                        ...
                        >
                            ...
                            <ActionButtonWrapper>
                                <RequestButton
                                    ...
                                    disabled={game.getRenderData([
                                        "requestButton",
                                        "disabled",
                                    ])}
                                    onClick={game.getRenderData([
                                        "requestButton",
                                        "onClick",
                                    ])}
                                >
        ...
        const addWindow = () => {
            this.windowManager.createWindow({
                ...
                children(props) {
                    return (
                        <div>
                            Hello there <br />X<br />X<br />X<br />X<br />X
                            <br />X
                        </div>
                    );
                },
        ...
        const fullScreenButtonContents = (
            <FlexRow style={{ flexShrink: 0 }}>
                ...
                <ArrowToolTip title="not_bad" placement="bottom">
                    <div
                        style={{ cursor: "pointer", fontSize: "2em" }}
                        onClick={() => game.sendSound("not_bad")}
                    >
                        🤔
                    </div>
                </ArrowToolTip>
                ...
                <ArrowToolTip title="Super genious" placement="bottom">
                    <div
                        style={{ cursor: "pointer", fontSize: "2em" }}
                        onClick={() => game.sendSound("super_genius")}
                    >
                        🎓
                    </div>
                </ArrowToolTip>
                ...
                <ArrowToolTip title="shazaam" placement="bottom">
                    <div
                        style={{ cursor: "pointer", fontSize: "2em" }}
                        onClick={() => game.sendSound("shazaam")}
                    >
                        ⚡️
                    </div>
                </ArrowToolTip>
                ...
                <ArrowToolTip title="Fart" placement="bottom">
                    <div
                        style={{ cursor: "pointer", fontSize: "2em" }}
                        onClick={() => game.sendSound("farting")}
                    >
                        🌬️
                    </div>
                </ArrowToolTip>
                        ...
                        <RelLayer>
                            <WindowContainer
                                ...
                                children={({ containerSize }) => {
                                    ++childIndex;
                                    windowManager.setContainerSize(
                                        containerSize
                                    );
                                                            ...
                                                            if (
                                                                ...
                                                                contents = (
                                                                    <DragWindow
                                                                        ...
                                                                        children={windowManager.getWindowChildren(
                                                                            window.id
                                                                        )}
                                                                        actions={
                                                                            window.actions
...
const mapStateToProps = (state) => ({
    ...
    getAllPeople: () => {
        return state.people.order.map(
            (personId) => state.people.items[personId]
        );
    },
