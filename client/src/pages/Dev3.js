...
import { isFunc, isDef, isDefNested, isArr, getNestedValue } from "../utils/";
...
import AcceptOrDeclineScreen from "../components/screens/AcceptOrDeclineScreen";
...
import PROPERTY_DATA from "../data/propertySets.js";
...
import actionButtonContents from "../components/ActionButtonContents";
...
class GameUI extends React.Component {
  ...
  async handleOnHandCardClick({ cardId }) {
    ...
    if (isGameOver) {
      ...
      if (isRequestScreenOpen) {
        ...
        let onClickAutoCollect = async ({ requestId, receive }) => {
          console.log(
            onClickAutoCollect,
            JSON.stringify({ requestId, receive }, null, 2)
          );
        ...
        game.updateRenderData(
          ...
          <RequestScreen
            ...
            propertySetsKeyed={PROPERTY_DATA.items}
  ...
  makeCardCheck({
    ...
    function makeOnSelectCard(cardId) {
      if (actionCardId === cardId) {
        return async () => {
          await game.cancelPropertyAction();
        };
      }
      if (isCollection) {
        if (isCollectionSelectable) {
  ...
  renderDiscardCardsNotice() {
    ...
    if (game.isMyTurn()) {
      if (game.cards.myHand.hasTooMany()) {
        turnNotice = (
          <TurnNotice>
            You have {game.cards.myHand.getTooMany()} too cards in your hand,
  ...
  renderPlayerPanel(person) {
    ...
    if (isDef(person)) {
      ...
      let playerSelection = "";
      if (game.selection.people.isEnabled()) {
        ...
        playerSelection = (
          <Checkbox
            ...
            value={isChecked}
            onChange={() => {
              game.togglePersonSelected(person.id);
            }}
          />
        );
      }
